...
class PRL(Trainer):
    ...
    def __init__(self, chamber, trainer_config = {}, trainer_config_file = '~/trainer_PRL_config.yaml'):
        ...
        self.config.ensure_param("high_reward_probability", 0.8)  # Probability of high reward
        self.config.ensure_param("low_reward_probability", 0.2)
        self.config.ensure_param("trial_to_reverse", 45)  # Trial number to switch the reward probability
    ...
    def run_training(self):
        ...
        if self.state == PRLState.IDLE:
            ...
            if self.current_trial < self.config["num_trials"]:
                ...
                logger.info(f"Reward probabilities set for trial {self.current_trial}: {self.chamber.touchscreen_1.get_reward_probability()}, {self.chamber.touchscreen_2.get_reward_probability()}")
