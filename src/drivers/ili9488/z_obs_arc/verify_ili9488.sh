...
# Adjust the patterns as needed: the example below excludes
# many typical Pi-specific warnings.
...
# Print or log the filtered dtc output (only lines that are *not* excluded)
...
# For node presence checks, we still want to see if dtc's *unfiltered* output
# or the actual /proc/device-tree has the node name. We'll keep the old
# logic, but any dtc warnings about the Pi base tree won't flood your console.
##############################################################################
