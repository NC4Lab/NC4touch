...
int main()
{
    ...
    // Define black and white colors (16-bit RGB565 format)
    ...
    std::cout << "Filling screen with white..." << std::endl;
    for (int i = 0; i < 480 * 320; ++i)
    {
        framebuffer.write(reinterpret_cast<char *>(&white), sizeof(white));
    }
    framebuffer.flush();
    std::cout << "Screen should now be white." << std::endl;
